...
export async function handleWebsocketDataUpdates(): Promise<number> {
...
export function handleAvailabilityUpdates() {
    const availability = useSettingsStore.getState().localAvailability ?? useUserStore.getState().availability;
