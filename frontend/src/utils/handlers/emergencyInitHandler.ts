...
export function emergencyInitHandler(event: MessageEvent) {
	const emergencyInitValidation = EmergencyInitPayloadSchema.safeParse(
		JSON.parse(event.data),
	);

	if (emergencyInitValidation.success) {
		const state = useEmergencyStore.getState();
		state.setEmergencyInitiatorId(
			emergencyInitValidation.data.initiatorClientDbId,
		);
		...
		errorLogger.logError(
			new Error("Failed to parse emergency init payload"),
		);
